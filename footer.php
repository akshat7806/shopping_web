...
<script>
   document.getElementById("four-button").addEventListener('click', () => {
      document.getElementById("five-button").classList.remove("active");
      document.getElementById("four-button").classList.add("active");
      const cards = document.getElementsByClassName('cards');
      console.log(cards);

      for (var i = 0; i < cards.length; i++) {
         cards[i].classList.remove("row-cols-xxl-5");
         cards[i].classList.remove("row-cols-xl-5");
         cards[i].classList.add("row-cols-xxl-4");
         cards[i].classList.add("row-cols-xl-4");
      }
   });
   document.getElementById("five-button").addEventListener('click', () => {
      document.getElementById("four-button").classList.remove("active");
      document.getElementById("five-button").classList.add("active");
      const cards = document.getElementsByClassName('cards');
      console.log(cards);

      for (var i = 0; i < cards.length; i++) {
         cards[i].classList.remove("row-cols-xxl-4");
         cards[i].classList.remove("row-cols-xl-4");
         cards[i].classList.add("row-cols-xxl-5");
         cards[i].classList.add("row-cols-xl-5");
      }
   ...
   function openTab(evt, tabName) {
      ...
      const bigger = document.getElementsByClassName("nav-all-tab");
      console.log(bigger);
      tabContent = document.getElementsByClassName("tab");
...
function filterItems(category) {
    ...
    $(".item").each(function() {
        if (category === "all" || $(this).data("category") === category) {
            $(this).show(); // Show the item if it matches the category or if "all" is selected
        } else {
            $(this).hide(); // Hide the item if it doesn't match the category
        }
    });
}
...
$(".filter-button").click(function() {
    var category = $(this).data("filter"); // Get the category from the data attribute
    filterItems(category); // Filter items based on the selected category
});
